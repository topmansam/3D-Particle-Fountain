...
/* variable to track if particle when particle should be removed if their speed is less
than this value */
...
void display() {
    ...
    for (int i = 0; i < particles.size(); i++) {
        ...
        glTexCoord2f(0.0f, 0.0f);
        glVertex3f(particles[i].x1, particles[i].y1, particles[i].z1);
        if (randomOn) glColor3f(particles[i].r, particles[i].g, particles[i].b);
        glTexCoord2f(0.0f, 1.0f);
        glVertex3f(particles[i].x2, particles[i].y2, particles[i].z2);
        if (randomOn) glColor3f(particles[i].r, particles[i].g, particles[i].b);
        glTexCoord2f(1.0f, 1.0f);
        glVertex3f(particles[i].x3, particles[i].y3, particles[i].z3);
...
int main(int argc, char** argv) {
    cout << "1 : Constant stream\n" <<
        ...
        "S : Random speed\n" <<
        "D : Random spin\n" <<
        "C : Random color and size\n" <<
        "G : Friction mode\n" <<
        "T : Spark on collision\n" <<
        "Q : High spray mode\n" <<
